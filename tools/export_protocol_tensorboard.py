...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Comma-separated protocol directories to export.",
    )
    parser.add_argument(
        "--models",
        default=",".join(MODELS),
...
def selected_protocols(args):
    return split_csv_arg(args.protocols)


def selected_models(args):
    return split_csv_arg(args.models)
...
def total_result_path(args, protocol, model):
    ...
    if model == "agcn":
        ...
        if not matches:
            raise FileNotFoundError(f"No AGCN result CSV found under {root}")
        return matches[0]
    matches = sorted(
        root.glob("clif*lamda_le_*_lamda_kd_relation_aff_*_lamda_kd_relation_data_*.csv"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not matches:
...
def experiment_label(protocol, model):
    ...
    return f"{protocol_label} {model_label}"
...
def run_name(protocol, model):
    protocol_label = PROTOCOL_LABELS.get(protocol, protocol)
    model_label = MODEL_LABELS.get(model, model.upper())
    return f"{safe_tag(protocol_label)}/{model_label}"
...
def collect_loss(args):
    rows = {}
    for protocol in selected_protocols(args):
        for model in selected_models(args):
            try:
                path = detail_result_path(args, protocol, model, "loss")
            ...
            with path.open(newline="") as handle:
                ...
                for row in csv.DictReader(handle):
                    record = {}
                    for key, value in row.items():
                        if key in {"trained_task", "epoch", "num_batches"}:
                            record[key] = int(value)
                        elif key in LOSS_METRIC_NAMES:
                            record[key] = float(value) if value else np.nan
                        else:
                            record[key] = value
                    records.append(record)
            rows[(protocol, model)] = records
...
def add_loss_scalars(writer, label, records):
    if not records:
        return

    task_ids = sorted({record["trained_task"] for record in records})
    for task_id in task_ids:
        task_records = [
            record for record in records if record["trained_task"] == task_id
        ]
        for metric in LOSS_METRIC_NAMES:
            values_by_epoch = {
                record["epoch"]: record.get(metric, np.nan)
                for record in task_records
            }
            if all(np.isnan(value) for value in values_by_epoch.values()):
                continue
            for epoch in sorted(values_by_epoch):
                value = values_by_epoch[epoch]
                writer.add_scalar(f"task{task_id}/{metric}", float(value), epoch)
                writer.add_scalar(f"{metric}/task{task_id}", float(value), epoch)
    ...
    writer.add_text(
        ...
        "\n".join(
            [
                ...
                "- loss_clf, loss_kd_logits, loss_le, loss_kd_model, loss_kd_aff, etc.: loss curves from *_loss_metrics.csv.",
                "",
                "Task group view:",
                "- task0/map",
                "- task0/microf1",
...
def export_tensorboard(args):
    ...
    loss_results = collect_loss(args)
    ...
    for protocol in selected_protocols(args):
        for model in selected_models(args):
            ...
            loss_records = loss_results[(protocol, model)]
            ...
            add_loss_scalars(writer, label, loss_records)
